...
## 每日营养与记录编辑（CAL-8 / CAL-12）

- 食品库支持每 100g 的蛋白质、膳食纤维、脂肪、碳水化合物。新食品不勾选“已知”表示 NULL/未知；勾选并填写 `0.00` 表示已知为零。已有兼容值按下述规则保留。存储和计算不按显示位数舍入，显示统一两位小数。
- 显式 v3 营养值逐项优先；NULL 时，仅迁移前食品（`created_at < schema v3 applied_at`）或来源明确的内置食品可回退已有营养，包括零。v3 新食品的旧默认零不表示已知。迁移和读取均不回填历史；编辑器只展示一组有效营养。
- CAL-12 对旧 v0.0.2 的一个已批准异常单独作启动修复：仅 FOOD/RECIPE、标记 0、四个 v3 快照全空且至少一个旧快照大于零的记录，先保存 `backups/pre_repair_cal12_nutrition_*.sqlite3` 独立安全快照，再原子复制其自身旧快照并设为完整。全零、CUSTOM 或部分已有 v3 值不修复；一般不完整语义不变。无命中不备份、不写入，快照失败则中止启动；详见 [CAL-12 修复说明](docs/CAL12_IMPLEMENTATION.md) 第 8 节。
- 每 100g 元数据按实际克数缩放；兼容字段按原 `basis_amount` / `basis_unit` 缩放，支持旧 ml 食品。`ml` 不等于 g，不猜测密度；显式质量元数据无法用于体积摄入时，该项保持未知。
...
- 摄入 `nutrition_complete=NULL` 时可逐项复用旧快照（包括零）；`=1` 读取完整新快照；`=0` 明确不完整，禁止用旧默认零补全。真正缺失数据时显示 **部分记录无营养数据**。全部未知显示“未知”，无记录日显示 `0.00 g`。
...
当前版本不包含联网食品库、AI/条码识别、云同步、账号、多用户、健康平台接入、自动运动估算、营养诊断、减重计划、Web 或移动端。内置食品参考值不构成医疗或营养诊断。营养与时间线编辑沿用既有精度保留输入控件。
