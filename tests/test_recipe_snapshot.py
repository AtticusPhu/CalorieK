...
class RecipeSnapshotTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.beef_id = self.foods.create_food(
            name="牛肉", category="肉类", basis_unit="g", kcal=250, protein_g=26
        )
        self.rice_id = self.foods.create_food(
            name="杂粮饭", category="主食", basis_unit="g", kcal=116, carb_g=25
        )
    ...
    def test_recipe_intake_snapshot_survives_ingredient_and_recipe_edits(self) -> None:
        ...
        self.assertAlmostEqual(preview["kcal"], 366.0)
        self.assertAlmostEqual(preview["per_100g_kcal"], 183.0)
        ...
        self.assertAlmostEqual(totals["kcal"], 366.0)
        self.assertAlmostEqual(totals["per_100g_kcal"], 183.0)
        ...
        self.assertAlmostEqual(float(original["kcal_snapshot"]), 366.0)
        ...
        self.foods.update_food(self.beef_id, kcal=230)
        ...
        self.assertAlmostEqual(float(historical["kcal_snapshot"]), 366.0)
        ...
        # Current recipe is 300 g and 576 kcal; 150 g records one half.
        self.assertAlmostEqual(float(new_event["kcal_snapshot"]), 288.0)
    ...
    def test_soft_deleted_sources_do_not_invalidate_existing_recipe_event(self) -> None:
        ...
        expected_kcal = float(self.foods.get_intake_event(event_id)["kcal_snapshot"])
        ...
        self.assertAlmostEqual(float(event["kcal_snapshot"]), expected_kcal)
    ...
    def test_mixed_mass_and_volume_are_kept_separate(self) -> None:
        ...
        self.assertEqual(totals["per_100g_kcal"], 0.0)
        ...
        self.assertAlmostEqual(float(event["kcal_snapshot"]), 83.0)
    ...
    def test_liquid_only_recipe_uses_ml_reference(self) -> None:
        ...
        self.assertAlmostEqual(totals["per_100g_kcal"], 60.0)
        event_id = self.recipes.record_recipe_intake(
            recipe_id, amount=125, unit="ml"
        )
        event = self.foods.get_intake_event(event_id)
        self.assertEqual(event["unit"], "ml")
        self.assertAlmostEqual(float(event["kcal_snapshot"]), 75.0)
